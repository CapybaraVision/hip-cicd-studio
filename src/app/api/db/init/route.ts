...
export async function GET() {
    try {
        ...
        const sheets = ['Pulse', 'VoC', 'Stats', 'Sprint', 'Projects', 'Pipeline'];
        const results = [];
        ...
        for (const title of sheets) {
            ...
            // Try to load headers to see if they exist
            let headersLoaded = false;
            try {
                await sheet.loadHeaderRow();
                headersLoaded = true;
            } catch (e) {
                // Headers likely missing or empty sheet
            }
            ...
            if (!headersLoaded || !sheet.headerValues || sheet.headerValues.length === 0) {
                if (title === 'Pulse') {
                    ...
                    await sheet.setHeaderRow(['id', 'title', 'status', 'assignee', 'priority']);
                } else if (title === 'Pipeline') {
                    await sheet.setHeaderRow(['id', 'service_id', 'version', 'status', 'trigger', 'author', 'time', 'features']);
                }
                results.push(`Initialized Headers for ${title}`);
            } else {
                results.push(`${title} has headers`);
            }
        }
